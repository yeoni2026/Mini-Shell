...
### 2. 내장 명령어
`cd`(`chdir()`), `pwd`(`getcwd()`), `exit`, `jobs` 네개의 내장 명령어를 구현했다. `cd`는 자식 프로세스가 아닌 쉘 프로세스 자신이 직접 `chdir()`을 호출해야 하는데, 이는 fork로 생성된 자식이 종료되면 그 프로세스의 작업 디렉토리 변경 사항도 함께 사라지기 때문이다.
...
3번 문제를 고친 이후, 입력이 COMMAND_SIZE를 넘으면 fgets()가 그 줄을 끝까지 못 읽고 나머지 글자들이 stdin에 그대로 남는다는 점이 눈에 들어왔다. 이 남은 부분을 비워주기 위해 매 루프마다 while ((c = getchar()) != EOF && c != '\n');을 실행하도록 넣었다. 그런데 이렇게 하니 ls 같은 평범한 명령어를 쳐도 다음 프롬프트가 아예 안 뜨고 멈춰버리는, 더 심각한 문제가 새로 생겼다. 곧 원인을 찾았는데 — 입력이 짧아서 개행 문자까지 이미 다 읽힌 정상적인 경우에도 이 while문이 무조건 실행되면서, stdin에 더 이상 비울 게 없는데도 계속 다음 입력을 기다리며 멈춰버리기 때문이었다.
